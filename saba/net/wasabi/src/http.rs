...
impl HttpClient {
    ...
    pub fn get(&self, host: String, port: u16, path: String) -> Result<HttpResponse, Error> {
        ...
        let mut stream = match TcpStream::connect(socket_addr) {
            Ok(stream) => stream,
            Err(_) => return Err(Error::Network("Failed to connect TCP stream".to_string())),
        };
        ...
        request.push_str("Connections: close\n");
        request.push_str("\n");
        ...
        match core::str::from_utf8(&received) {
            ...
            Err(e) => Err(Error::Network(format!("Invalid receive response: {}", e))),
        }
    }
}
