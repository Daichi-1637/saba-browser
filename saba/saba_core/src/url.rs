...
impl Url {
    ...
    fn extract_host(&self) -> String {
        ...
        if let Some(index) = url_parts.first().unwrap().find(':') {
            // indexよりも前の文字列を取得
            url_parts.first().unwrap()[..index].to_string()
        } else {
            url_parts.first().unwrap().to_string()
        }
    }
    ...
    fn extract_port(&self) -> String {
        ...
        if let Some(index) = url_parts.first().unwrap().find(':') {
            // index+1よりも後の文字列を取得
            url_parts.first().unwrap()[index + 1..].to_string()
        } else {
            "80".to_string()
        }
    }
    ...
    fn extract_path(&self) -> String {
        ...
        path_and_searchpart.first().unwrap().to_string()
    }
